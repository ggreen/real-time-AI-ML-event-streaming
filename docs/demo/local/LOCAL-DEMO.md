...
```shell
./deployments/local/data-services/postgres/start-postgresml.sh
```

Start RabbitMQ

```shell
./deployments/local/data-services/rabbit/start-rabbitmq.sh
```

Start ValKey

```shell
./deployments/local/data-services/valkey/start.sh
```
...
```shell
java -jar applications/web-ai-app/target/web-ai-app-0.0.1-SNAPSHOT.jar --ai.vector.similarity.search.threshold=0.8 --spring.profiles.active=amqp --mail.user.id=queeen
```
--------------
# Testing
